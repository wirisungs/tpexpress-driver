...
const CompletedOrdersScreen: React.FC = () => {
  ...
  const fetchCompletedOrders = async () => {
    ...
    try {
      ...
      const response = await fetch('http://10.0.2.2:3000/order/completed', {
        headers: {
  ...
  return (
    <StyledView className="flex-1 bg-white">
      ...
      <StyledView className="flex-row justify-around py-4">
        ...
        >
          <StyledText className="text-gray-500 font-semibold text-base">Đơn giao thất bại</StyledText>
        </StyledTouchableOpacity>
        <StyledTouchableOpacity
          className="flex-1 justify-center items-center px-2 py-1"
          onPress={() => navigation.navigate('CompletedOrdersScreen' as never)}
        >
          <StyledText className="text-orange-500 font-semibold text-base">Đơn đã hoàn thành</StyledText>
        </StyledTouchableOpacity>
      </StyledView>
    </StyledView>
  );
